...
class OpenLcaData:
    ...
    def make_vehicle_type_lca_params_beb(
        ...
        return _create_vehicle_type_lca_params_beb(
    ...
    def make_vehicle_type_lca_params_diesel(
        self,
        overrides: VehicleTypeOverrides,
    ) -> VehicleTypeLcaParams:
        ...
        return _create_vehicle_type_lca_params_diesel(
            chassis_ef_per_kg=self.chassis_per_kg,
    ...
    def make_battery_type_lca_params(
        self,
        chemistry: str | None,
    ) -> BatteryTypeLcaParams:
        ...
        return _create_battery_type_lca_params(
            emission_factors_per_kg=ef,
            battery_lifetime_years=self.battery_lifetime_years,
        )

    def make_charging_point_type_lca_params(
        self,
        ...
        return _create_charging_point_type_lca_params(
...
def populate_lca_params_from_data(
    ...
    for vtype in vehicle_types:
        ...
        if name_short not in vehicle_type_overrides:
            warnings.warn(
                ...
                f"and will have no lca_params set.",
                stacklevel=2,
            )
            continue
        ...
        if vtype.energy_source == EnergySource.BATTERY_ELECTRIC:
            params = d.make_vehicle_type_lca_params_beb(year, ovr)
        elif vtype.energy_source == EnergySource.DIESEL:
            params = d.make_vehicle_type_lca_params_diesel(ovr)
        else:
    ...
    # (driven by fleet.json). This function only writes lca_params on rows
    # that already exist.
    battery_types = (
        session.query(BatteryType).filter(BatteryType.scenario_id == scenario_id).all()
    )
    for bt in battery_types:
        bt.lca_params = d.make_battery_type_lca_params(
    ...
    for cpt in charging_point_types:
        ...
        cpt.lca_params = d.make_charging_point_type_lca_params(cpt_overrides).to_dict()
...
def init_lca_params(
    ...
    """Populate ``lca_params`` on all eflips-model entities for a scenario.
    ...
    with create_session(scenario, database_url) as (session, scenario_obj):
        ...
        if missing:
            warnings.warn(
                ...
                f"{overrides_json_path.name}; lca_params will not be written.",
                UserWarning,
                stacklevel=2,
            )
            return
